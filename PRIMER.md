...
For use in prompts where Unicode operators cannot appear, these ASCII names map to Unicode operators:
...
Prompts describing a pattern using these ASCII names will output the corresponding Unicode operators.
...
`ARG1` through `ARG9` in text are shorthand for numbered placeholder positions. Rules for ARGn markers:
